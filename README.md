...
- **Tool-use approval, two surfaces (opt-in, off by default since 0.3.4).** Set `tool_approval = "ask"` in `config.toml` to enable the gate; while it's on, when `hook-pretool` decides a tool is risky the broker fans out a `tool_request` event in parallel to Discord (`✅ Allow` / `❌ Deny` button card) and the local tray (Windows toast with the same action buttons via the `agentmux://` URL scheme). The hook long-polls `/tool-request` for up to 5 minutes; whichever endpoint POSTs `/tool-decision/:id` first wins, broker idempotently 404s the loser. **Discord auto-silences itself when a local viewer (`claude-attach` or web) is attached** — broker still emits the event so tray fires its toast, but the IM card is suppressed because you're already at a screen and don't need a phone ping too. The event payload carries `local_viewer_attached: bool` so any other approval surface can apply the same rule. The default flipped to off because Discord-driven workflows hit too many approvals to stay usable; with `--dangerously-skip-permissions` already in `default_command`, claude itself doesn't prompt — turn the gate back on if you want a second human-in-the-loop layer.
...
- **Default command** is `claude --dangerously-skip-permissions`. The PreToolUse approval flow is intended to *replace* claude's own permission prompts with a more flexible regex-driven one — but as of 0.3.4 it's off by default (`tool_approval = "off"`); set `tool_approval = "ask"` in `config.toml` to re-enable. If you'd rather use claude's built-in prompts instead, drop `--dangerously-skip-permissions` from `default_command` and leave the agentmux gate off.
